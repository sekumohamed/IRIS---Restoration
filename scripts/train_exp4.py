...
Experiment 4: same architecture and loss as Experiment 3 (IRISStronger +
CombinedLoss), but trained on AugmentedIRISDataset instead of the plain
real-pairs dataset -- adding order-agnostic synthetic degradation
augmentation (see degradation_simulator.py) on top of everything that
already worked in Experiment 3.

This isolates augmentation as the single new variable:
    Experiment 3: IRISStronger + CombinedLoss, real pairs only       -> 29.05 dB / 0.7946 (clean)
    Experiment 4: IRISStronger + CombinedLoss, + synthetic aug (this) -> ?

Validation is ALWAYS real pairs only (via make_train_val_split's val_set
directly, never wrapped in AugmentedIRISDataset) so the comparison
against Experiment 3 stays apples-to-apples -- only the training data
mix changes, not what "good" is measured against.

Usage:
    python train_exp4.py --data_root "C:\\Users\\sekuh\\Desktop\\semicon\\train" --epochs 100 --batch_size 8
"""
...
from augmented_dataset import AugmentedIRISDataset
...
def main():
    parser = argparse.ArgumentParser(description="Train IRISStronger + CombinedLoss + synthetic augmentation (Experiment 4)")
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="checkpoints_exp4")
    ...
    parser.add_argument("--synthetic_prob", type=float, default=0.3,
                         help="Fraction of training samples replaced with synthetic order-randomized pairs per epoch")
    ...
    augmented_train_set = AugmentedIRISDataset(train_set, synthetic_prob=args.synthetic_prob)

    print(f"Train pairs: {len(augmented_train_set)} (synthetic_prob={args.synthetic_prob}) | Val pairs (real only): {len(val_set)}")

    train_loader = DataLoader(
        augmented_train_set, batch_size=args.batch_size, shuffle=True,
    ...
    print(f"(Exp 3, no augmentation, was: 26.62 dB batch-pooled / 29.05 dB clean per-image)")
