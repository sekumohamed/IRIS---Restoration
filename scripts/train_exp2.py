...
Experiment 2 (per report Section 10/15 ablation plan): baseline
IRISBaseline architecture, UNCHANGED, trained with CombinedLoss
(Charbonnier + SSIM + Edge) instead of plain Charbonnier.

This isolates the effect of the loss function alone. Same model class,
same dataset split (seed=42), same optimizer/scheduler setup as
train.py -- the ONLY variable that changes is the loss. That is what
makes this a valid ablation entry: "Experiment 1 (baseline) vs
Experiment 2 (baseline + structural + edge loss)".

Saves to a SEPARATE checkpoint directory (checkpoints_exp2/) so the
Phase 2 baseline checkpoint is never overwritten -- we need both for
the final comparison table and demo.

Usage:
    python train_exp2.py --data_root "C:\\Users\\sekuh\\Desktop\\semicon\\train" --epochs 30
"""
...
def compute_ssim(pred: torch.Tensor, target: torch.Tensor) -> float:
    """Same fixed-window SSIM metric as train.py, kept identical so
    val_psnr/val_ssim numbers are directly comparable across experiments."""
    pred_clipped = pred.clamp(0.0, 1.0)
...
def main():
    parser = argparse.ArgumentParser(description="Train IRISBaseline with CombinedLoss (Experiment 2)")
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="checkpoints_exp2")
    ...
    print(f"(Baseline Experiment 1 was: 25.76 dB / SSIM 0.7673 -- compare against that)")
